...
alpha_vantage_schema = {
    ...
    "NEWS_SENTIMENT": {
        ...
        "topics": ["blockchain", "earnings", "ipo", "mergers_and_acquisitions",
                   ...
                   "retail_wholesale", "technology", "string (comma-separated topics)"], # Also allows comma-separated combinations
    ...
    "ANALYTICS_FIXED_WINDOW": {
        ...
        "RANGE": ["full", "{N}day", "{N}week", "{N}month", "{N}year",
                  "{N}minute", "{N}hour", # Intraday only
                  "YYYY-MM-DD", "YYYY-MM", # Specific dates/months
                  "YYYY-MM-DDTHH:MM:SS"], # Specific datetime, can specify start & end with two RANGE params
        "OHLC": ["open", "high", "low", "close"],
        "INTERVAL": ["1min", "5min", "15min", "30min", "60min", "DAILY", "WEEKLY", "MONTHLY"],
        "CALCULATIONS": ["MIN", "MAX", "MEAN", "MEDIAN", "CUMULATIVE_RETURN",
                         "VARIANCE", "VARIANCE(annualized=True)",
                         "STDDEV", "STDDEV(annualized=True)",
                         "MAX_DRAWDOWN", "HISTOGRAM", "HISTOGRAM(bins=N)",
                         "AUTOCORRELATION", "AUTOCORRELATION(lag=N)",
                         "COVARIANCE", "COVARIANCE(annualized=True)",
                         "CORRELATION", "CORRELATION(method=KENDALL)", "CORRELATION(method=SPEARMAN)",
    ...
    "ANALYTICS_SLIDING_WINDOW": {
        ...
        "CALCULATIONS": ["MEAN", "MEDIAN", "CUMULATIVE_RETURN",
                         "VARIANCE", "VARIANCE(annualized=True)",
                         "STDDEV", "STDDEV(annualized=True)",
                         "COVARIANCE", "COVARIANCE(annualized=True)",
                         "CORRELATION", "CORRELATION(method=KENDALL)", "CORRELATION(method=SPEARMAN)",
    ...
    "DIGITAL_CURRENCY_DAILY": {
        ...
        "datatype": ["json", "csv"] # Note: Doc implies only JSON/CSV but examples show datatype=csv usage
    },
